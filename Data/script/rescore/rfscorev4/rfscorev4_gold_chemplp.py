...
def rfscore_perform(pdbname, ligname, label, i, return_dict):
    mylist = []
    molname = \
    open('%s/%s_%s_gold_chemplp/%s/%s_top1.sdf' % (pdbname, pdbname, label, ligname, ligname), 'r').readlines()[
        0].split('|')[0].strip()
    for n in range(1, 4):
        if not os.path.exists(
                        '%s/%s_%s_gold_chemplp/%s/%s_top%s.pdbqt' % (pdbname, pdbname, label, ligname, ligname, n)):
        ...
        if os.path.exists('%s/%s_%s_gold_chemplp/%s/%s_top%s.mol2' % (pdbname, pdbname, label, ligname, ligname, n)):
            cmdline = 'cd %s/%s_%s_gold_chemplp/%s &&' % (pdbname, pdbname, label, ligname)
...
def interagte_result(pdbname, label):
    ...
    pool.join()
    mycsv = open(
        '%s/%s_%s_gold_chemplp/%s_%s_gold_chemplp_rfscore4_score.csv' % (pdbname, pdbname, label, pdbname, label),
        'w')
