...
def NNscore2_perform(pdbname, ligname, label, i, return_dict):
    mylist = []
    ...
    for n in range(1, 4):
        ...
        if os.path.exists('%s/%s_%s_glide_SP/%s/%s_top%s.pdbqt' % (pdbname, pdbname, label, ligname, ligname, n)):
            cmdline = 'cd %s/%s_%s_glide_SP/%s &&' % (pdbname, pdbname, label, ligname)
            cmdline += 'python2 /home/shenchao/AI_evaluation/software/NNscore2/NNScore2.01.py -receptor ../../%s_prot/%s_p.pdbqt -ligand %s_top%s.pdbqt' % (
            ...
            for k in range(1, 21):
                ...
                for line in lines:
                    if line.startswith('Best pose scored by network #%s:' % k):
                        scores['molscore_net%s' % k] = line.split(':')[-1].split('=')[1].strip()
            molscore_best = None
            for line in lines:
                if line.startswith('              BEST SCORE OF ALL 20 NETWORKS, BY POSE '):
                    molscore_best = lines[lines.index(line) + 3].split('|')[2].strip()
            molscore_ave = None
            for line in lines:
                if line.startswith('              AVERAGE SCORE OF ALL 20 NETWORKS, BY POSE '):
                    molscore_ave = lines[lines.index(line) + 3].split('|')[2].strip()
					
        score_list = [molname, ligname, 'top%s' % n, molscore_best, molscore_ave]
        for k in range(1, 21):
...
def interagte_result(pdbname, label):
    ...
    for ligname in lignames:
        ...
        p = pool.apply_async(NNscore2_perform, args=(pdbname, ligname, label, im, return_dict))
        jobs.append(p)
    pool.close()
    pool.join()
    mycsv = open('%s/%s_%s_glide_SP/%s_%s_glide_SP_NNscore2_score.csv' % (pdbname, pdbname, label, pdbname, label),
                 'w')
    mycsvwriter = csv.writer(mycsv)
    header = ['molname', 'ligid', 'topn', 'molscore_best', 'molscore_ave']
    for k in range(1, 21):
