...
def rfscore_perform(pdbname, ligname, label, i, return_dict):
    ...
    for n in range(1, 4):
        ...
        if os.path.exists('%s/%s_%s_ledock/%s/%s_top%s.mol2' % (pdbname, pdbname, label, ligname, ligname, n)):
            cmdline = 'cd %s/%s_%s_ledock/%s &&' % (pdbname, pdbname, label, ligname)
...
def interagte_result(pdbname, label):
    ...
    pool.close()
    pool.join()
    mycsv = open('%s/%s_%s_ledock/%s_%s_ledock_rfscore4_score.csv' % (pdbname, pdbname, label, pdbname, label),
                 'w')
