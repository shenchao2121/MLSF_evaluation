...
def deltavina_perform(pdbname, ligname, label, i, return_dict):
    ...
    for n in range(1, 4):
        ...
        os.system(cmdline)
        molscore = open('%s/%s_%s_gold_chemplp/%s/output.csv' % (pdbname, pdbname, label, ligname), 'r').readlines()[1].split(',')[-1].strip()
    ...
    for filename in filenames:
        if os.path.basename(filename) not in ['total_chemplp.sdf', '%s_top1.sdf' % ligname, '%s_top2.sdf' % ligname, '%s_top3.sdf' % ligname]:
...
def interagte_result(pdbname, label):
    ...
    pool.close()
    pool.join()
    mycsv = open('%s/%s_%s_gold_chemplp/%s_%s_gold_chemplp_deltavina_score.csv' % (pdbname, pdbname, label, pdbname, label),
                 'w')
