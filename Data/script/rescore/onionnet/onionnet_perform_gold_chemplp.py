...
def onionnet_perform(pdbname, ligname, label, i, return_dict):
    ...
    for n in range(1, 2):
        ...
        os.system(cmdline)
        molscore = \
        open('%s/%s_%s_gold_chemplp/%s/%s_top%s_onionnet/output.csv' % (pdbname, pdbname, label, ligname, ligname, n),
...
def interagte_result(pdbname, label):
    ...
    pool.close()
    pool.join()
    mycsv = open('%s/%s_%s_gold_chemplp/%s_%s_gold_chemplp_onionnet_score.csv' % (pdbname, pdbname, label, pdbname, label),
                 'w')
