...
def rfscorevs_perform(pdbname, ligname, label, i, return_dict):
    ...
    for n in range(1, 4):
        ...
        os.system(cmdline)
        molscore = open('%s/%s_%s_glide_SP/%s/%s_top%s.csv' % (pdbname, pdbname, label, ligname, ligname, n), 'r').readlines()[1].strip()
        mylist.append([molname, ligname, 'top%s' % n, molscore])

    os.remove('%s/%s_%s_glide_SP/%s/%s_top1.csv' % (pdbname, pdbname, label, ligname, ligname))
    os.remove('%s/%s_%s_glide_SP/%s/%s_top2.csv' % (pdbname, pdbname, label, ligname, ligname))
    os.remove('%s/%s_%s_glide_SP/%s/%s_top3.csv' % (pdbname, pdbname, label, ligname, ligname))
...
def interagte_result(pdbname, label):
    ...
    pool.close()
    pool.join()
    mycsv = open('%s/%s_%s_glide_SP/%s_%s_glide_SP_rfscorevs_score.csv' % (pdbname, pdbname, label, pdbname, label),
                 'w')
