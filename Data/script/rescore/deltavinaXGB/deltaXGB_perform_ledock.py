...
def deltavina_perform(pdbname, ligname, label, i, return_dict):
    ...
    for n in range(1, 2):
        ...
        cmdline += '--runfeatures --datadir %s/%s/%s_%s_ledock/%s/%s_top%s_cal --pdbid %s --average' % (
        os.getcwd(), pdbname, pdbname, label, ligname, ligname, n, pdbname)
        os.system(cmdline)
        molscore_vina = \
        open('%s/%s_%s_ledock/%s/%s_top%s_cal/score.csv' % (pdbname, pdbname, label, ligname, ligname, n),
...
def interagte_result(pdbname, label):
    ...
    pool.close()
    pool.join()
    mycsv = open('%s/%s_%s_ledock/%s_%s_ledock_deltaXGB_score.csv' % (pdbname, pdbname, label, pdbname, label),
                 'w')
