...
def pafnucy_perform(pdbname, ligname, label, i, return_dict):
    ...
    for n in range(1, 4):
        ...
        if os.path.exists('%s/%s_%s_glide_SP/%s/%s_top%s.sdf' % (pdbname, pdbname, label, ligname, ligname, n)):
            ...
            os.system(cmdline)
            molscore = None
            if os.path.exists('%s/%s_%s_glide_SP/%s/%s_top%s_pafnucy.csv'%(pdbname, pdbname, label, ligname, ligname, n)):
			    molscore = open('%s/%s_%s_glide_SP/%s/%s_top%s_pafnucy.csv'%(pdbname, pdbname, label, ligname, ligname, n), 'r').readlines()[1].split(',')[-1].strip()
            mylist.append([molname, ligname, 'top%s' % n, molscore])
    return_dict[i] = mylist

    os.remove('%s/%s_%s_glide_SP/%s/%s_top1.hdf' % (pdbname, pdbname, label, ligname, ligname))
    os.remove('%s/%s_%s_glide_SP/%s/%s_top1_pafnucy.csv' % (pdbname, pdbname, label, ligname, ligname))
    os.remove('%s/%s_%s_glide_SP/%s/%s_top2.hdf' % (pdbname, pdbname, label, ligname, ligname))
    os.remove('%s/%s_%s_glide_SP/%s/%s_top2_pafnucy.csv' % (pdbname, pdbname, label, ligname, ligname))
    os.remove('%s/%s_%s_glide_SP/%s/%s_top3.hdf' % (pdbname, pdbname, label, ligname, ligname))
    os.remove('%s/%s_%s_glide_SP/%s/%s_top3_pafnucy.csv' % (pdbname, pdbname, label, ligname, ligname))
...
def interagte_result(pdbname, label):
    ...
    pool.close()
    pool.join()
    mycsv = open('%s/%s_%s_glide_SP/%s_%s_glide_SP_pafnucy_score.csv' % (pdbname, pdbname, label, pdbname, label),
                 'w')
