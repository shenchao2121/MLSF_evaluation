...
def deltavina_perform(pdbname, ligname, label, i, return_dict):
    ...
    for n in range(1, 2):
        ...
        cmdline += '--runfeatures --datadir %s/%s/%s_%s_gold_chemplp/%s/%s_top%s_cal --pdbid %s --average' % (
        os.getcwd(), pdbname, pdbname, label, ligname, ligname, n, pdbname)
        os.system(cmdline)
        molscore_vina = \
        open('%s/%s_%s_gold_chemplp/%s/%s_top%s_cal/score.csv' % (pdbname, pdbname, label, ligname, ligname, n),
    ...
    cmdline = 'cd %s/%s/%s_%s_gold_chemplp/%s &&' % (os.getcwd(), pdbname, pdbname, label, ligname)
...
def interagte_result(pdbname, label):
    ...
    pool.close()
    pool.join()
    mycsv = open('%s/%s_%s_gold_chemplp/%s_%s_gold_chemplp_deltaXGB_score.csv' % (pdbname, pdbname, label, pdbname, label),
                 'w')
